...
export class SettingsElementsListView {
  ...
  constructor(configElements: ConfigurationElement[], onConfigurationChanged: () => void = () => { }) {
    this.onConfigurationChanged = onConfigurationChanged;
    this.list.replaceChildren(
      ...configElements.map(element => configElementToDiv(element, this.onConfigurationChanged.bind(this))));
  }

  public getConfigElements(): ConfigurationElement[] {
    return Array.from(this.list.children).map(child => divToConfigElement(child as HTMLDivElement));
  }

  public addConfigElement(configElement: ConfigurationElement): void {
    this.list.appendChild(configElementToDiv(configElement, this.onConfigurationChanged.bind(this)));
  }
}

function configElementToDiv(configElement: ConfigurationElement, onConfigurationChanged: () => void = () => { }): HTMLDivElement {
...
function divToConfigElement(div: HTMLDivElement): ConfigurationElement {
